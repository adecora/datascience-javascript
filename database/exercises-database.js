...
class Database {
  ...
  // Exercise2. Filtering records.
  getLongerThan (args, callback) {
    this.db.all(Q_WORKSHOP_LONGER_THAN, args, (err, rows) => {
      if (err) this._fail(err)
      callback(rows)
    })
  }
  ...
  // Exercise3. More filtering.
  getWithinLengthRange(args, callback) {
    this.db.all(Q_WORKSHOP_DURATION_RANGE, args, (err, rows) => {
      if (err) this._fail(err)
      callback(rows)
    })
  }
  ...
  addOne (args, callback) {
    const self = this
    self.db.run(Q_WORKSHOP_ADD, args, function (err, rows) {
      if (err) self._fail(err)
